...
def ejecutar_tarea(dataset_q, dataset_r, datos_temporales, filename_resultados):
    # comandos para calcular descriptores
    comando1 = ["python", "tarea1-indexar.py", dataset_r, datos_temporales]
    ejecutar(comando1)
    # comandos para buscar
    comando2 = ["python", "tarea1-buscar.py", dataset_q, datos_temporales, filename_resultados]
    ejecutar(comando2)

def evaluar_en_dataset(nombre):
    print()
    print("------- {} -------".format(nombre))
    if not os.path.isdir(nombre):
        print("no existe {}".format(nombre))
        sys.exit(1)
    dataset_q = "{}/q".format(nombre)
    dataset_r = "{}/r".format(nombre)
    ...
    if not os.path.isfile(filename_gt):
        ...
        sys.exit(1)
    datos_temporales = "datos_temporales_{}".format(nombre)
    filename_resultados = "resultados_{}.txt".format(nombre)
    ejecutar_tarea(dataset_q, dataset_r, datos_temporales, filename_resultados)
    metric = evaluar_resultado_en_dataset(nombre, filename_gt, filename_resultados)
    return metric

def calcular_nota(resultado_promedio, metrica_para_7):
    if resultado_promedio < metrica_para_7:
        nota = round(7 * resultado_promedio / metrica_para_7, 1)
        bonus = 0
        if nota < 1:
            nota = 1
    else:
...
def evaluar_tarea1(datasets):
    print("CC5213 - 2023a - Tarea 1")
    resultados = {}
    for name in datasets:
        nombre = "dataset_" + name
        resultado = evaluar_en_dataset(nombre)
        resultados[nombre] = resultado
    print()
    print("--------------------------------------------")
    print("Resumen:")
    for nombre in resultados:
        print("    F1-score en {}: {:.3f}".format(nombre, resultados[nombre]))
    promedio = numpy.average(list(resultados.values()))
    metrica_para_7 = 0.65
    nota, bonus = calcular_nota(promedio, metrica_para_7)
    print("    ==> Promedio F1-score: {:.3f}".format(promedio))
    print()
    print("    Para obtener nota 7.0 se requiere F1-score {:.3f}".format(metrica_para_7))
...
datasets = ["a"]
if len(sys.argv) > 1 and "--full" == sys.argv[1]:
